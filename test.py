...
from yolov7obb.utils.general import coco80_to_coco91_class, check_dataset, check_file, check_img_size, check_requirements, \
    box_iou, non_max_suppression, scale_coords, xyxy2xywh, xywh2xyxy, set_logging, increment_path, colorstr
from yolov7obb.utils.metrics import ap_per_class, ConfusionMatrix
from yolov7obb.utils.plots import plot_images, output_to_target, plot_study_txt
from yolov7obb.utils.torch_utils import select_device, time_synchronized, TracedModel

# gwang add
from yolov7obb.utils.rboxs_utils import rbox2poly, poly2hbb
...
def test(
    ...
    model=None,
    dataloader=None,
    work_directory=Path(''),  # for saving images
    ...
    if training:  # called by train.py
        ...
        # Directories
        work_directory = Path(increment_path(Path(opt.project) / opt.name, exist_ok=opt.exist_ok))  # increment run
        (work_directory / 'labels' if save_txt else work_directory).mkdir(parents=True, exist_ok=True)  # make dir
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            # print("out after non_max_suppression_obb=", out)
        ...
        # Plot images
        if (plots or plots_debug) and batch_i < 10:
            f = work_directory / f'test_batch_{epoch}_{batch_i}_labels.jpg'  # labels
            Thread(target=plot_images, args=(img, targets, paths, f, names), daemon=True).start()
            f = work_directory / f'test_batch_{epoch}_{batch_i}_pred.jpg'  # predictions
            Thread(target=plot_images, args=(img, output_to_target(out), paths, f, names), daemon=True).start()

    # Compute statistics
    stats = [np.concatenate(x, 0) for x in zip(*stats)]  # to numpy
    if len(stats) and stats[0].any():
        p, r, ap, f1, ap_class = ap_per_class(*stats, plot=plots, save_dir=work_directory, names=names)
    ...
    # Plots
    if plots:
        confusion_matrix.plot(save_dir=work_directory, names=list(names.values()))
        if wandb_logger and wandb_logger.wandb:
            val_batches = [wandb_logger.wandb.Image(str(f), caption=f.name) for f in sorted(work_directory.glob('test*.jpg'))]
    ...
    if save_json and len(jdict):
        ...
        pred_json = str(work_directory / f"{w}_predictions.json")  # predictions json
    ...
    if not training:
        s = f"\n{len(list(work_directory.glob('labels/*.txt')))} labels saved to {work_directory / 'labels'}" if save_txt else ''
        print(f"Results saved to {work_directory}{s}")
